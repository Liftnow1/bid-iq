...
- **OPERATOR DECISION (Paul, 2026-05-29, AskUserQuestion):** **Leave gates as-is.** Conservative matching (catches near-identical re-proposals) is accepted; the shared DM webhook is NOT to be retuned. Logged as a closed decision — future sessions should treat the dilution behavior as a *known, accepted* limitation and NOT re-propose changing the shared endpoint unless Paul reopens it.
...
## ⭐ CAPSTONE — OPERATOR ACTION LEDGER (end of overnight run, 2026-05-29 UTC)
*This single list supersedes the scattered "decisions/flags" in #173/#174/#175 — read THIS, act, done. Everything below is operator-gated (your decision) or operator-only (n8n UI). I left all of it untouched per the rails.*

### 🔴 DECISIONS ONLY YOU CAN MAKE — prioritized
1. **HIGH — Approval Executor Backlink external-email branch** (`hpgbBAmRmqtsfr6g`, ACTIVE). For any APPROVED (stage-2) `Backlink Outreach` ticket it auto-sends a **real external email to `info@<target_domain>`** from `partnerships@liftnow.com` — the only external-email path in the executor; conflicts with the "no real email outside paulj@" rail. **Contained now:** 0 Backlink tickets at stage 2 + producer (Agent 6) is OFF, so nothing fires. **Latent:** fires the moment you approve a Backlink ticket while the executor is on. → *Recommend:* switch that branch to draft-to-`paulj@` (`auto_ack`), matching LinkedIn/Reddit — UNLESS you actually want autonomous outreach. **Until you decide, don't approve any Backlink Outreach ticket.** (Active wf — I did not touch it.)
2. **MED — SEM Auto-Optimizer activation policy** (Agent 15, `rZW6J0ccvzWjDxYr`, OFF). It's a *genuinely gated* autonomous Google-Ads-spend agent — `$100/day` total cap, ≤25%/change, 1-change-per-budget-per-day cooldown, and a "Needs-You" HubSpot ticket for changes above the auto threshold — but the auto path still spends real money with no per-change human sign-off. **Stays OFF until you explicitly authorize ad spend.** If/when you turn it on, consider routing *all* mutates through the same approval queue the dashboard drives. (Agent 4 SEM is read-only — no mutate node — so it's spend-safe regardless.)
3. **MED — Phase 5 Quote Watcher** (`fX21AsfCPF5FhMIj`, OFF, 42 nodes). Out-of-scope *sales-ops* pipeline: polls OneDrive PDFs → Claude-parses → auto-creates HubSpot **Companies + Quote objects + Line Items**. Structurally sound, but auto-creating CRM records is a real side effect I won't validate-toward-activation while you're away. → *Decide:* give it a dedicated review+enable, or fold it into the kill-list.
4. **LOW — Ticket housekeeping:** decide holdback ticket `45635364612`; delete test ticket `45620973745`.

### 🗑️ OPERATOR-ONLY DELETIONS (n8n UI — I never call the DELETE API)
**Confirmed-superseded → safe to delete (each has a live v2/v3 replacement, all paused):**
`77rXTw8CdOIFbtW6` OLD UI/UX v1 (→ Eagle v3) · `EnDUo4FL853PCNlo` Content Producer v1 (→ v2) · `DG8TpaVnn8fIjlJB` Keyword Discovery v1 (→ v2) · `JHG1YburLOORHcfC` LinkedIn Cadence v1 (→ v2) · `XAb1uPWPZgNdTe9Z` SEM Manager v1 (→ v2) · `QQDrSn84luEZVp05` Competitor SERP Monitor v1 (→ v2) · `gFk7wNiOSx39Mzf6` SEO Optimizer v1 (→ Bee v3) · `dOzXskoAG16triVQ` + `bR89sj5tK6Rcy7SY` Backlink Builder v1 ×2 (→ v2) · `riG3espgO00QYCEl` Backlink Poster 6b v1 · `7svFpGyXB0hWbADo` CF7 dup (active one is `P7c1fwvrmRV5CTrd`) · `5OnUsvIYa6YNk0iC` + `xZCWfSlfZEZYXuOV` Weekly Summary ×2 · `BDRaqnlzKM2lv3iA` + `CA5BEshVLXj4Z9Ry` Rank Tracker ×2.
**TEMP/test (explicitly disposable):** `K36Cm0bJEqTaPkfF` SMTP Test · `H39Qw8jyRn7QRfNl` TEMP Delete Tickets · `ij0C0RXFpFn32agW` TEMP GA4 · `6VVQwQnJ6hRna2rv` TEMP WP Plugin · `85NSPxIMzJ0SFZ2p` TEMP 7b helper · `pNEpr1PLVKn3WCPP` TEMP enum options.
**⚠️ VERIFY supersession BEFORE deleting (don't want you nuking something load-bearing):** `Zufhtq3fu4AJR6an` Content Dup Sentinel (likely replaced by Content Producer's internal dedup) · `CsPth7e5pqRdBDln` Duplicate Workflow Detector (was Build-C; possibly still useful) · `sM4zVVBpqvX21dZK` Daily Health Check (likely replaced by Silent Failure Detector + Vercel Watchdog) · `QBaTurTGBiAo113f` Site Audit Monitor v1. — I left these in a separate bucket on purpose; confirm each is truly dead before removing.

### 🏷️ COSMETIC — verify when convenient, nothing blocks
Cron *labels* that lie about cadence (behavior is fine, only the display name is wrong): ROI "Daily Midnight ET"=hourly · LinkedIn "M/W/F 10:30 ET"=Mon–Fri@10&14 (+orphaned `Auto-Post LinkedIn` node, posting not wired) · Backlink "Tue 9AM ET"=Mon–Fri@9&14 · Brand Listening "Every 30 Min"(verify) · Agent 6c "M/W/F 11AM"=every 2h · Agent 7 "Daily 10AM"=Mon–Fri@9/12/15/18 · Agent 10 "Daily 7AM"=every 2h · Agent 13 "Daily 7:45AM"=hourly. Also: Agent 12 `Execute Auto-Fixes` node name overpromises (it's proposal-only MVP) · Crow `recommendation_type="Reddit Reply"` shared with Community Engagement — confirm HubSpot enum.
